...
class Mascotas:
    ...
    def aniadir_organizacion(self, organizacion):
        check_nombre = self.comprobar_variable(organizacion['nombre'], str)
        check_ciudad= self.comprobar_variable(organizacion['ciudad'], str)
        check_pais = self.comprobar_variable(organizacion['pais'], str)
        
        if (check_nombre or check_ciudad or check_pais):
            return "Datos incorrectos para la nueva organización"
        
        (Mascotas.organizaciones)[Mascotas.ID_organizacion] = {
            'nombre':organizacion['nombre'],
            'ciudad':organizacion['ciudad'],
            'pais':organizacion['pais']
        }
        Mascotas.ID_organizacion += 1
        return Mascotas.organizaciones